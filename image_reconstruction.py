...
def load_image_data(image_name):
    ...
    if hasattr(skimage.data, image_name):
        ...
        raise Exception(f'Image name is wrong. Skimage does not have image with the name "{image_name}".')
...
def main():
    ...
    if os.getenv("WANDB_LOG") in ["true", "True", True]:
        run_name = (
            f'{nonlin}_{args.input_image}_image_denoise__{str(time.time()).replace(".", "_")}'
        )
    ...
    plt.savefig(
        os.path.join(
            os.getenv("RESULTS_SAVE_PATH"), "denoising", f"{nonlin}_{args.input_image}.png"
        )
    )
