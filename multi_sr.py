...
if __name__ == "__main__":
    ...
    os.makedirs(
        os.path.join(os.getenv("RESULTS_SAVE_PATH"), f"{nonlin}_multi_sr"),
        exist_ok=True,
    )
    ...
    # saving the model
    os.makedirs(
        os.path.join(os.getenv("MODEL_SAVE_PATH"), f"{nonlin}_multi_sr"),
        exist_ok=True,
    )
    torch.save(
        model.state_dict(),
        os.path.join(
            os.getenv("MODEL_SAVE_PATH"), f"{nonlin}_multi_sr", f"{nonlin}.pth"
        ),
    )
