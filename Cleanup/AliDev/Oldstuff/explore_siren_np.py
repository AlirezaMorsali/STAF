...
class SincLayer(nn.Module):
    ...
    def forward(self, input):
        inter = self.omega_0 * self.linear(input)
        result = np.cos(inter.detach().numpy())
    ...
    def bacward(self, x):
        # result = 6 * self.omega_0 * np.log(x.detach().numpy())
        # return torch.as_tensor(result, dtype=x.dtype)
        return self.omega_0 * torch.cos(x)
...
model_input2, ground_truth2 = model_input.cpu(), ground_truth.cpu()
step = 0
