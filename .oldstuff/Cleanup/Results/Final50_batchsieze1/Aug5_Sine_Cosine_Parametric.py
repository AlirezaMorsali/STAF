...
for counter in range(len(models)):
    ...
    history = models[counter].fit(dataset, epochs=epochs, callbacks=[callbacks[counter]])
...
for save_result in callbacks:
  Video.append((save_result.images))
  #psnr_buff = []
...
f = os.path.join(f'training_convergence{var_EPOCHS}.mp4')
...
plt.savefig(f'Compresion_on_{var_EPOCHS}.pdf', format='pdf', dpi=600, bbox_inches='tight', pad_inches=0)
...
# plt.savefig(f'Fixed_PSNR_Compresion_500.pdf', format='pdf', dpi=600, bbox_inches='tight',
#             pad_inches=0)
plt.savefig(f'PSNR_Compresion_{var_EPOCHS}.pdf', format='pdf', dpi=600, bbox_inches='tight',
            pad_inches=0)
saved_data = []

saved_data = [model_psnr, model_name, Video]

with open(f"data_{var_EPOCHS}.data", "wb") as f:
