...
img_relu = Relim(in_features=2, out_features=1, hidden_features=128, 
                  hidden_layers=1, outermost_linear=True)
img_relu.cuda()

img_sincren = Sincren(in_features=2, out_features=1, hidden_features=128, 
...
optim_3 = torch.optim.Adam(lr=1e-4, params=img_relu.parameters())
...
for step in range(total_steps):
    ...
    model_output_3, coords_3 = img_relu(model_input)    
    loss_3 = ((model_output_3 - ground_truth)**2).mean()
    ...
    if not step % steps_til_summary:
        ...
        axes[0].imshow(model_output_3.cpu().view(imsize,imsize).detach().numpy(),cmap='gray')
        axes[0].set_title('Relu')
        axes[1].imshow(model_output.cpu().view(imsize,imsize).detach().numpy(),cmap='gray')
        axes[1].set_title('Sine')
        axes[2].imshow(model_output_2.cpu().view(imsize,imsize).detach().numpy(),cmap='gray')
        axes[2].set_title('Parametric')
