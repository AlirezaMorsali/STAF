...
image = utils.normalize(plt.imread(args.input).astype(np.float32), True)
k = 2
im = cv2.resize(image, None, fx=1 / k, fy=1 / k, interpolation=cv2.INTER_AREA)
if len(im.shape) == 2:
    H, W = im.shape
    D = 1
    cmap = "gray"
...
model1 = utils.get_model(args.inr_model1, im, D, pos_encode_no)
model2 = utils.get_model(args.inr_model2, im, D, pos_encode_no)


optim1 = utils.get_optim(args.inr_model1, model1, args.lr1, args.maxpoints, H, W)
optim2 = utils.get_optim(args.inr_model2, model2, args.lr2, args.maxpoints, H, W)
