...
k = 1
im_orig = cv2.resize(im, None, fx=1 / k, fy=1 / k, interpolation=cv2.INTER_AREA)
im = im_orig
if len(im.shape) == 2:
    H, W = im.shape
    D = 1
...
orig_gt = torch.tensor(im).reshape(H * W, D)[None, ...].to(device)

# Initialize a tensor for reconstructed data
rec = torch.zeros_like(orig_gt)

# indices = torch.randperm(H * W)
indices = torch.arange(H * W)
# indices = torch.arange(H * W)[::2]
...
def train_epoch(
    ...
    # Calculate PSNR
    with torch.no_grad():
        model_output = model(coords.to(device))
        rec[:, indices, :] = model_output

        mse_array[step] = (
            ((orig_gt[:, indices, :] - rec[:, indices, :]) ** 2).mean().item()
        )
...
gt1 = (
    torch.tensor(im[::2, ::2]).reshape(H // 2 * W // 2, D)[None, ...].to(device).clone()
)
tind_1 = indices.reshape(H, W)[::2, ::2].flatten()
gt2 = (
    torch.tensor(im[1::2, 1::2])
    .reshape(H // 2 * W // 2, D)[None, ...]
    .to(device)
    .clone()
)
tind_2 = indices.reshape(H, W)[1::2, 1::2].flatten()
gt3 = (
    torch.tensor(im[1::2, ::2])
    .reshape(H // 2 * W // 2, D)[None, ...]
    .to(device)
    .clone()
)
tind_3 = indices.reshape(H, W)[1::2, ::2].flatten()
gt4 = (
    torch.tensor(im[1::2, ::2])
    .reshape(H // 2 * W // 2, D)[None, ...]
    .to(device)
...
tind_4 = indices.reshape(H, W)[1::2, ::2].flatten()
...
for step in tqdm(range(args.niters)):
    if step % 4:
        gt_train = gt1
        train_indices = tind_1
    elif step + 1 % 4:
        gt_train = gt2
        train_indices = tind_2
    elif step + 2 % 4:
        gt_train = gt3
        train_indices = tind_3
    elif step + 3 % 4:
        gt_train = gt3
        train_indices = tind_3
    # else:
...
fig_data.savefig("./results/PSNR.png")
